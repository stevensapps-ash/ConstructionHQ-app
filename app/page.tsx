...
function Invoices({data,setData,initialProject,onInitialProjectHandled}:{data:AppData;setData:(d:any)=>void;initialProject?:Project|null;onInitialProjectHandled?:()=>void}){
  ...
  function selectProject(name:string){setProject(name);const p=data.projects.find((x:any)=>x.name===name);if(!p)return;setCustomer(String(p.customer||''));const est=[...(data.estimates||[])].reverse().find((x:any)=>x.project===name);const planDoc=[...(data.docs||[])].reverse().find((x:any)=>(x.type==='Build Plan'&&x.buildPlan?.project===name)||(x.approvedEstimate?.project===name));const plan=planDoc?.buildPlan||planDoc?.approvedEstimate;const detailed:any[]=[];if(plan){(plan.supplies||[]).forEach((s:any,i:number)=>detailed.push({id:Date.now()+i,description:'Material — '+String(s.item||'Supply'),qty:Number(String(s.quantity||'').match(/[0-9]+(?:\\.[0-9]+)?/)?.[0]||1),rate:String(Number(s.estimatedUnitCost||0))}));if(Number(plan.laborCost||0)>0)detailed.push({id:Date.now()+1000,description:'Labor'+((plan.laborTasks||[]).length?' — '+plan.laborTasks.join('; '):''),qty:Number(plan.estimatedLaborHours||1)||1,rate:String(Number(plan.estimatedLaborHours||0)>0?Number(plan.laborCost||0)/Number(plan.estimatedLaborHours):Number(plan.laborCost||0))});(plan.steps||[]).forEach((s:any,i:number)=>{if(String(s||'').trim())detailed.push({id:Date.now()+2000+i,description:'Work completed — '+String(s).trim(),qty:1,rate:'0'})})}if(detailed.length){setItems(detailed);return}const amount=Number(est?.amount||p.amount||0);if(amount>0)setItems([{id:Date.now(),description:p.scope||name+' — contracted project work',qty:1,rate:String(amount)}])}
